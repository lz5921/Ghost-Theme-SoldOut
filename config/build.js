...
fs.readFile(`${rootDir}/default.hbs`, "utf8", (err, data) => {
  ...
  const result = data.replace(/<%keywords%>/, config.网站关键词);
...
fs.readFile(`${rootDir}/partials/site-footer.hbs`, "utf8", (err, data) => {
  ...
  const result = data
    ...
    .replace(
      /<a href="http:\/\/beian.miit.gov.cn" target="_blank" rel="nofollow noopener">苏ICP备15050739号<\/a>/,
      config.页脚自定义链接1
    );
  // .replace(
  //   /<a href="http:\/\/www.beian.gov.cn\/portal\/registerSystemInfo?recordcode=32010402000196" target="_blank" rel="nofollow noopener">苏公网安备32010402000196号<\/a>/,
  //   config.页脚自定义链接2
  // )
  // .replace(
  //   /<a href="https:\/\/tongji.baidu.com\/web\/welcome\/ico?s=075de8ed5c2bfb5f678b1a620749aee4" target="_blank" rel="nofollow noopener">百度统计<\/a>/,
  //   config.页脚自定义链接3
  // );
...
fs.readFile(`${rootDir}/index.hbs`, "utf8", (err, data) => {
  ...
  const result = data.replace(/永远年轻，永远热泪盈眶！/, config.博客欢迎语);
...
fs.readFile(`${rootDir}/custom-about-for-page.hbs`, "utf8", (err, data) => {
  ...
  fs.writeFile(
    `${rootDir}/custom-about-for-page.hbs`,
    result,
    "utf8",
    (err) => {
      if (err) {
